...
export const CreateBreed = () => {
 ...
 useEffect(() => {
  setValues({...formValues, temperament: newTemps.map( temp => temp.value ).join(', ')})
 }, [newTemps])
  ...
  const formValidation = () => {
    ...
    if( !name.match(/^[a-zA-Z_ ]*$/) ){
      dispatch(uiActions.setError('Name must have only letters or cant be blank'))
      return false
    } else if(height <= 1 || weight <= 1 || life_span <= 1 ){
      dispatch(uiActions.setError('Height, Weight or Life expactancy must be higher than 1'))
      return false
  ...
  return (
    <div className='create__breed-container container'>
      ...
      {
          errorMessage &&
           (<div className='create__breed-container-error__message'>{ errorMessage }</div>) 
      }
      <form onSubmit={ handleFormSubmit } className="create__breed-container-form">
        ...
        <fieldset>
          ...
          <label>Height:</label>
          <input type="number" name="height"  value={ height } onChange={ handleInputChange } required/>
          <label>Weight:</label>
          <input type="number" name="weight"  value={ weight } onChange={ handleInputChange } required/>
          <label>Image url:</label>
          <input type="text" name="image"  value={ image } onChange={ handleInputChange } required/>
          <label>Life Expectancy:</label>
          <input type="number" name="life_span"  value={ life_span } onChange={ handleInputChange } required/>
          ...
          <div className='create__breed-option-container'>
            {
              newTemps.map( (newTemp, i) => (
                <div key={ i }>
                  <p>{ newTemp.value }</p>
                  <button
                    type= "button" onClick={ () => handleDelete(newTemp.id) }
                  >x</button>
                </div>
              ) )
            }
